...
library(logspline)
    ...
    new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
        ...
        lapply(new.packages, require, character.only=T)
            ...
            linearData <- function(Dataset) {
                if(!file.exists(Dataset))
                stop("file '", Dataset, "'does not exist")
                        data <-({ suppressMessages({
                        readr::read_csv(Dataset, progress = FALSE)
        })
                dplyr::tbl_df(data)
        })
  }
...
#' @name pweibull
#' @description Computation for 'wiebull' cumulative probability distribution
#' @param g A number
#' @param m3 A number
#' @return plot
#' @export
...
cat("\n")
cat("Clinical significance is determined from the probability curve for SPRE predictions,if any 2 predictions from the second significant digit (after the decimal place) remains constant. Otherwise, there is no clinical significance and the vertical line remains zero.")

cat("\n", "\n")
cat("SPRE Weibull prediction, K-S statistic under the null: ")
...
#' @name n.sims
#' @description K-S estimation to determine sufficient scale of the data measure
#' @param r A function
#' @param m4 A number
#' @return computation
...
cat("\n")
        ...
        m4 <-  print(1 - plogspline(ks.test(z
                                    , "pweibull"
                                    , shape= fit.weibull$estimate["shape"]
                                    , scale = fit.weibull$estimate["scale"])$statistic
                            , fit
        ...
        title(paste(main = "SPRE Weibull Prediction, K-S Pstat. =", m4, "\n",       "(K-S alpha=0.05, n=13,14:measure threshold=0.36,0.35)","\n","\n","\n"))
